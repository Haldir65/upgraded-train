...
function _build_grpc(){
    ...
    *)
    ...
    # 5. 打包安装后的目录
    cd "$BUILD_DIR"
    echo "--- 正在打包 grpc_static_${ARCH_SUFFIX}.zip ---"
    # 使用 -r 递归打包整个目录
    mv grpc_static grpc
    zip -r grpc_static_${ARCH_SUFFIX}.zip grpc

    # 6. 上传到 HTTP Server
    cd "$ROOT_DIR"
    mv $BUILD_DIR/grpc_static_${ARCH_SUFFIX}.zip .
    UPLOAD_URL="https://${UPLOAD_API_URL}"

    echo "--- 正在上传 grpc_static_${ARCH_SUFFIX}.zip 到 backend ---"
    curl -X POST \
        -F "file=@grpc_static_${ARCH_SUFFIX}.zip" \
